...
\* Task 3 MuJoCo: the environment landed in [`task3_mujoco/`](task3_mujoco/) — the 100- and 300-bean scenes compile and the robot is in the scene, so rows 1 and 2 are checked. Keyboard teleoperation, force-limited grasping, and a full four-stage run are **not** yet checked: they need a maintainer to drive the interactive session on a machine with a display. What is verified so far is the headless suite — 14 of 15 validation scripts pass, covering grasp/lift, fingertip contact, spine dynamics, and the wall-collision guard; the 15th fails for a pre-existing upstream reason recorded in [`task3_mujoco/README.md`](task3_mujoco/README.md#known-issues). Twenty visual assets are hosted outside git; see that README's fetch step. Tracked in [#14](https://github.com/EBiM-Benchmark/benchmark/issues/14).
