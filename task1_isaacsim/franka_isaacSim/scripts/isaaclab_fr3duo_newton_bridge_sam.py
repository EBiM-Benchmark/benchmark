...
def _create_cable_stage_visuals(
    ...
    if board_usd.is_file():
        ...
        if not translation_attr or not translation_attr.IsValid():
            raise RuntimeError(f"Expected existing xformOp:translate on {board_xform.GetPrim().GetPath()}")
        if not rotate_attr or not rotate_attr.IsValid():
            raise RuntimeError(f"Expected existing xformOp:rotateZYX on {board_xform.GetPrim().GetPath()}")
