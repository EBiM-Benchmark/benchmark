...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--usd-path",
        # Rigid-finger variant (reverted default): inner joints welded to FIXED at
        # their authored pose, knuckle limits [12,47] deg to stop tip crossing.
        # Pass ..._mimic.usd for the baked-mimic variant, or plain ..._v0_2.usd raw.
        default="/workspace/franka_isaacSim/assets/Robotiq_2f_85_with_d405_mobile_fr3_duo_v0_2_rigidfinger.usd",
    ...
    parser.add_argument(
        ...
        help="franka_isaacSim repository mount inside the IsaacLab container.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Rigid-finger variant (default): inner joints welded FIXED, only the "
            "outer-knuckle DOFs are driven. Use with --usd-path ..._rigidfinger.usd."
        ),
    )
    parser.add_argument(
        "--gripper-mimic",
        ...
        default=False,
        help=(
            "Baked-mimic gripper (needs --usd-path ..._mimic.usd and --no-gripper-weld): "
            "drivers driven, followers passive + hard-coupled by baked "
            "PhysxMimicJointAPI. Off by default (reverted to the weld variant)."
        ),
    )
...
# MIMIC design (chosen, matches the reference FR3_robotiq_2f85.usd): one driven
# joint per gripper (*_right_finger_joint, keeps its DriveAPI) with every other
# linkage joint hard-coupled to it via a BAKED PhysxMimicJointAPI constraint.
# Newton honors this when it is baked into the USD at load time (the reference
# proves it; our earlier runtime patch failed only on timing).
#
# `ratio` = software-coupling ratio (follower = ratio * driver), used only by the
# --no-gripper-mimic software fallback. The baked mimic uses gearing = -ratio
# (Newton convention: follower = -gearing * driver). Flip a ratio to flip both.
GRIPPER_COUPLING = {
    "left": {
        "driver": LEFT_GRIPPER_DRIVER,
        "followers": {"left_outer_knuckle_joint": 1.0},
    },
    "right": {
        "driver": RIGHT_GRIPPER_DRIVER,
        "followers": {"right_outer_knuckle_joint": 1.0},
    },
}

# Inner revolute joints welded to FIXED so each finger is a rigid solid. Only the
# outer-knuckle joints (the drivers/mirrors above) remain as DOFs.
GRIPPER_WELD_JOINTS = [
...
def _weld_robotiq_redundant_joints(robot_prim_path: str) -> None:
    """Convert the Robotiq inner revolute joints to FIXED (weld) joints.

    The inner joints aren't enforced as hard constraints on this backend, so the
    links separate/dangle. Welding them makes each finger a single rigid solid
    (only the outer-knuckle joint remains a DOF) and merges the massless
    outer_knuckle with its finger links so the composite body has real inertia.
    """
    stage = omni.usd.get_context().get_stage()
    if stage is None:
        print("Warning: cannot weld gripper joints: no USD stage", file=sys.stderr)
        return
    robot_prim = stage.GetPrimAtPath(robot_prim_path)
    if not robot_prim.IsValid():
        print(f"Warning: cannot weld gripper joints: robot prim not found: {robot_prim_path}", file=sys.stderr)
        return

    wanted = set(GRIPPER_WELD_JOINTS)
    welded = []
    seen = set()
    for prim in _iter_prims_under(robot_prim):
        if prim.GetName() not in wanted:
        ...
        if "Joint" not in str(prim.GetTypeName()):
            print(f"Warning: cannot weld {prim.GetPath()}: not a joint ({prim.GetTypeName()})", file=sys.stderr)
            continue
        if prim.GetTypeName() != "PhysicsFixedJoint":
            # Re-typing the prim welds body0/body1 at their current relative pose.
            # Leftover revolute axis/limit attrs are ignored by a fixed joint.
            prim.SetTypeName("PhysicsFixedJoint")
            welded.append(str(prim.GetPath()))

    missing = wanted - seen
    if missing:
        print("Warning: weld joint names not found: " + ", ".join(sorted(missing)), file=sys.stderr)
    if welded:
        print("Welded Robotiq inner joints to FIXED (rigid fingers):")
        for joint_path in welded:
            print(f"  {joint_path}")


def _axis_to_rot_token(prim):
    from pxr import UsdPhysics  # noqa: PLC0415

    attr = prim.GetAttribute("physics:axis")
    axis = str(attr.Get()) if attr and attr.IsValid() else "Z"
    return {
        "X": UsdPhysics.Tokens.rotX,
        "Y": UsdPhysics.Tokens.rotY,
        "Z": UsdPhysics.Tokens.rotZ,
    }.get(axis, UsdPhysics.Tokens.rotZ)


def _add_robotiq_mimic_joints(robot_prim_path: str) -> None:
    """Apply hard PhysxMimicJointAPI coupling to the Robotiq follower joints.

    The USD imports the 2F-85 as an open tree with only *_right_finger_joint
    driven and no coupling, so followers dangle. The real gripper is a mimic
    design (see robotiq_2f_85_macro.urdf.xacro). This recreates that with a hard
    solver constraint: q_follower = gearing * q_driver, using the multipliers in
    GRIPPER_COUPLING. Followers are left passive in the actuator config.
    """
    try:
        from pxr import PhysxSchema  # noqa: PLC0415
    except Exception as exc:  # pragma: no cover - schema missing on some backends
        print(f"Warning: PhysxSchema unavailable, skipping gripper mimic: {exc}", file=sys.stderr)
        return

    stage = omni.usd.get_context().get_stage()
    if stage is None:
        print("Warning: cannot apply gripper mimic: no USD stage", file=sys.stderr)
        return
    robot_prim = stage.GetPrimAtPath(robot_prim_path)
    if not robot_prim.IsValid():
        print(f"Warning: cannot apply gripper mimic: robot prim not found: {robot_prim_path}", file=sys.stderr)
        return

    joints_by_name = {}
    for prim in _iter_prims_under(robot_prim):
        if "Joint" in str(prim.GetTypeName()):
            joints_by_name.setdefault(prim.GetName(), prim)

    applied = []
    for spec in GRIPPER_COUPLING.values():
        driver_prim = joints_by_name.get(spec["driver"])
        if driver_prim is None:
            print(f"Warning: mimic driver joint not found: {spec['driver']}", file=sys.stderr)
            continue
        driver_path = driver_prim.GetPath()
        ref_axis = _axis_to_rot_token(driver_prim)
        for follower_name, gearing in spec["followers"].items():
            follower_prim = joints_by_name.get(follower_name)
            if follower_prim is None:
                print(f"Warning: mimic follower joint not found: {follower_name}", file=sys.stderr)
                continue
            own_axis = _axis_to_rot_token(follower_prim)
            mimic = PhysxSchema.PhysxMimicJointAPI.Apply(follower_prim, own_axis)
            rel = mimic.CreateReferenceJointRel()
            rel.SetTargets([driver_path])
            mimic.CreateReferenceJointAxisAttr().Set(ref_axis)
            mimic.CreateGearingAttr().Set(float(gearing))
            mimic.CreateOffsetAttr().Set(0.0)
            applied.append(f"{follower_name} <- {spec['driver']} (gearing={gearing:+g})")

    if applied:
        print("Applied Robotiq mimic joints:")
        for entry in applied:
            print(f"  {entry}")
    else:
        print("Warning: no Robotiq mimic joints were applied", file=sys.stderr)


def _gripper_driver_names() -> List[str]:
    return [spec["driver"] for spec in GRIPPER_COUPLING.values()]


def _gripper_follower_names() -> List[str]:
    return [name for spec in GRIPPER_COUPLING.values() for name in spec["followers"]]


def _make_gripper_actuators(use_mimic: bool, use_weld: bool = False) -> Dict[str, ImplicitActuatorCfg]:
    if use_weld:
        # Rigid-finger design: only the outer-knuckle joints remain as DOFs (the
        # ROS drivers + their software-mirrored partners). The welded inner joints
        # have no DOF, so they must NOT appear in any actuator group.
        return {
            "gripper_drivers": ImplicitActuatorCfg(
                joint_names_expr=_gripper_driver_names() + _gripper_follower_names(),
                # Softer + velocity-limited so a large open/close STEP command
                # can't apply an explosive torque to the light rigid finger.
    ...
    if use_mimic:
        # Hard PhysxMimicJointAPI (applied in _add_robotiq_mimic_joints) makes the
        # followers dependent DOFs, so they must be PASSIVE here — a competing
        # position drive would fight the mimic constraint. Only the true drivers
        # (*_right_finger_joint) get gains.
        return {
            "gripper_drivers": ImplicitActuatorCfg(
                joint_names_expr=_gripper_driver_names(),
                # Only the drivers are actuated; the baked mimic drags the rest.
                # velocity_limit prevents a large step command from exploding the
                # coupled linkage.
            ...
            "gripper_mimic_followers": ImplicitActuatorCfg(
                joint_names_expr=_gripper_follower_names(),
                # Passive: stiffness/damping 0 => no drive torque; the mimic
                # constraint (not this actuator) enforces the follower angles.
                stiffness=0.0,
                damping=0.0,
            ),
        }
    # Fallback: software position-servo coupling (see apply_commands). Followers
    # need real gains because nothing else constrains them.
    return {
        "gripper_drivers": ImplicitActuatorCfg(
            joint_names_expr=[
        ...
        "gripper_inner_knuckles": ImplicitActuatorCfg(
            joint_names_expr=[
                "left_left_inner_finger_knuckle_joint",
                "left_right_inner_finger_knuckle_joint",
                "right_left_inner_finger_knuckle_joint",
        ...
        "gripper_pads": ImplicitActuatorCfg(
            joint_names_expr=[
                "left_left_inner_finger_joint",
                "left_right_inner_finger_joint",
                "right_left_inner_finger_joint",
    ...
    robot_cfg = ArticulationCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            ...
            articulation_props=sim_utils.ArticulationRootPropertiesCfg(
                # Override the solver iteration counts baked into the USD.
                # Higher position iterations stabilize the soft-driven Robotiq
                # gripper linkage during contact (reduces jitter / penetration).
        ...
        actuators={
            ...
            # Robotiq grippers: welded rigid fingers (default), hard mimic, or
            # software-coupling fallback.
            **_make_gripper_actuators(args_cli.gripper_mimic, args_cli.gripper_weld),
        },
    )
...
class IsaacLabRosBridge(Node):
    def __init__(self, groups: List[JointGroup], *, enable_cable: bool = False):
        ...
        self._gripper_coupling_cache = None
    ...
    def apply_commands(self, robot, group_indices: Dict[str, Dict[str, int]]):
        ...
        # Robotiq four-bar coupling: the USD has no mimic/loop-closure, so mirror
        # the commanded driver angle onto its follower joints. Only runs for a
        # gripper whose driver was actually commanded (keeps arm targets intact).
        self._apply_gripper_coupling(robot, target)
    ...
    def _resolve_gripper_coupling(self, robot):
        """Build (and cache) driver/follower joint indices for each gripper."""
        if self._gripper_coupling_cache is not None:
            return self._gripper_coupling_cache
        names = list(robot.data.joint_names)
        name_to_idx = {name: idx for idx, name in enumerate(names)}
        resolved = {}
        for side, spec in GRIPPER_COUPLING.items():
            driver_idx = name_to_idx.get(spec["driver"])
            if driver_idx is None:
                print(
                    f"Warning: gripper coupling driver not found: {spec['driver']} ({side})",
                    file=sys.stderr,
                )
            followers = []
            for follower_name, ratio in spec["followers"].items():
                follower_idx = name_to_idx.get(follower_name)
                if follower_idx is None:
                    print(
                        f"Warning: gripper coupling follower not found: {follower_name} ({side})",
                        file=sys.stderr,
                    )
                    continue
                followers.append((follower_idx, float(ratio)))
            resolved[side] = {
        ...
        self._gripper_coupling_cache = resolved
        return resolved

    def _apply_gripper_coupling(self, robot, target) -> None:
        # When hard PhysxMimicJointAPI coupling is active the solver enforces the
        # follower angles, so software servoing would only fight it.
        if getattr(args_cli, "gripper_mimic", False):
            return
        coupling = self._resolve_gripper_coupling(robot)
        for info in coupling.values():
            driver_idx = info["driver_idx"]
            if driver_idx is None:
                continue
            # Only couple when this gripper's driver was actually commanded, so
            # we never overwrite arm/base targets on frames with no gripper cmd.
            if info["driver"] not in self.latest_commands.get(info["label"], {}):
                continue
            theta = target[:, driver_idx]
            for follower_idx, ratio in info["followers"]:
                target[:, follower_idx] = ratio * theta
...
def main():
    ...
    if args_cli.gripper_weld:
        _weld_robotiq_redundant_joints(robot_prim_path)
    # NOTE: gripper mimic is BAKED into the mimic USD (see bake_gripper_mimic.py),
    # not patched at runtime — the runtime PhysxMimicJointAPI patch is ignored by
    # Newton (timing), so it is intentionally not called here.
    sim.reset()
    scene.reset()
