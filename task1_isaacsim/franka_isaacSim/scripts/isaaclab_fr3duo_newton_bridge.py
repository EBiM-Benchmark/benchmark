...
def _load_scaled_joint_drive_configs(franka_root: Path, embodiment: str) -> dict[str, dict]:
    if yaml is None:
        print("Warning: PyYAML unavailable; using fallback actuator gains.", file=sys.stderr)
        return {}
    ...
    if not joint_drive_config_path.exists():
        print(f"Warning: joint drive config not found: {joint_drive_config_path}", file=sys.stderr)
        return {}
    ...
    if not drive_path.exists():
        print(f"Warning: Isaac joint drive config not found: {drive_path}", file=sys.stderr)
        return {}
    ...
    arm_stiffness_scale = float(arm_scaling.get("stiffness_scale", 0.65))
    arm_damping_scale = float(arm_scaling.get("damping_scale", 1.0))
    arm_max_force_scale = float(arm_scaling.get("max_force_scale", 3.0))
    gripper_stiffness_scale = float(gripper_scaling.get("stiffness_scale", 1.0))
    gripper_damping_scale = float(gripper_scaling.get("damping_scale", 1.0))
    gripper_max_force_scale = float(gripper_scaling.get("max_force_scale", 1.0))

    scaled = {}
    for joint_name, cfg in (drive_config.get("joint_drives", {}) or {}).items():
        if not isinstance(cfg, dict):
            continue
        is_gripper = "robotiq" in str(joint_name).lower() or "finger" in str(joint_name).lower()
        stiffness_scale = gripper_stiffness_scale if is_gripper else arm_stiffness_scale
        damping_scale = gripper_damping_scale if is_gripper else arm_damping_scale
        max_force_scale = gripper_max_force_scale if is_gripper else arm_max_force_scale
        scaled[str(joint_name)] = {
            "stiffness": float(cfg.get("stiffness", 0.0)) * stiffness_scale,
            "damping": float(cfg.get("damping", 0.0)) * damping_scale,
            "max_force": float(cfg.get("max_force", 0.0)) * max_force_scale,
        }

    print(
        "Loaded Isaac joint drive config for IsaacLab actuators: "
        f"{drive_path} (arm scales: k={arm_stiffness_scale:g}, d={arm_damping_scale:g}, "
        f"force={arm_max_force_scale:g}; gripper scales: k={gripper_stiffness_scale:g}, "
        f"d={gripper_damping_scale:g}, force={gripper_max_force_scale:g})"
    )
    return scaled
...
def _make_actuator_cfgs(franka_root: Path, embodiment: str) -> dict[str, ImplicitActuatorCfg]:
    ...
    arm_joint_names = LEFT_FALLBACK_JOINTS + RIGHT_FALLBACK_JOINTS
    missing_arm_drives = []
    for joint_name in arm_joint_names:
        drive = scaled_drives.get(joint_name)
        if drive is None:
            missing_arm_drives.append(joint_name)
            continue
        actuators[f"arm_{joint_name}"] = ImplicitActuatorCfg(
            joint_names_expr=[joint_name],
            effort_limit_sim=float(drive["max_force"]),
            stiffness=float(drive["stiffness"]),
            damping=float(drive["damping"]),
        )

    if missing_arm_drives:
        print(
            "Warning: missing arm joint drive entries; using fallback shared arm actuator for: "
            + ", ".join(missing_arm_drives),
            file=sys.stderr,
        )
        actuators["arms_fallback"] = ImplicitActuatorCfg(
            joint_names_expr=missing_arm_drives,
            effort_limit_sim=400.0,
            stiffness=800.0,
            damping=80.0,
        )

    # Match the Robotiq main finger drives authored in
    # Robotiq_2f_85_with_d405_mobile_fr3_duo_v0_2.usd.
    gripper_drive = {"stiffness": 3.0, "damping": 0.0002, "max_force": 26.0}
    actuators["grippers"] = ImplicitActuatorCfg(
        joint_names_expr=[".*(finger|knuckle).*"],
        effort_limit_sim=float(gripper_drive["max_force"]),
        stiffness=float(gripper_drive["stiffness"]),
        damping=float(gripper_drive["damping"]),
    )
    return actuators
    ...
    robot_cfg = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            joint_pos=_load_home_joint_positions(
                Path(args_cli.franka_root).expanduser(),
                args_cli.embodiment,
            )
        ),
        actuators=_make_actuator_cfgs(Path(args_cli.franka_root).expanduser(), args_cli.embodiment),
    )
...
def main():
    ...
    robotiq_finger_selectors = _default_robotiq_finger_selectors()
