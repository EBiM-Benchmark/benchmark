"""Camera configuration loading and normalization for Isaac Sim cameras.

This module handles loading camera sensor configurations from YAML files in
embodiment folders, normalizing parameters, and validating against data contracts.
"""
...
def resolve_camera_config_path(config_path: str | None = None) -> str:
    """Resolve camera config path, handling workspace prefixes and defaults.
    
    Args:
        config_path: Path to camera config YAML, or None for default
        
    Returns:
        Absolute path to resolved camera configuration file
    """
...
def load_camera_sensor_config(config_path: str | None = None) -> dict:
    """Load camera sensor configuration from YAML file.
    ...
    Raises:
        ...
        ValueError: If config file is not a valid mapping
    """
...
def _normalized_resolution(camera_config: dict) -> tuple[int, int]:
    """Extract normalized render resolution from camera config."""
...
def _normalized_aliases(key: str, camera_config: dict) -> tuple[str, ...]:
    """Build deduplicated alias list for camera lookups."""
    aliases = []
...
def focal_length_mm_from_hfov(sensor_width_mm: float, horizontal_fov_deg: float) -> float:
    """Calculate focal length from horizontal field of view and sensor width.
...
def frame_skip_count_for_rate(*, publish_hz: float, render_hz: float) -> int:
    """Calculate frame skip count to achieve desired publish rate.
    
    Args:
        publish_hz: Desired publishing frequency
        render_hz: Rendering frequency
        
    Returns:
        Number of frames to skip between publishes
    """
...
def normalize_camera_specs(config: dict) -> list[dict]:
    """Normalize camera specifications from configuration dict.
    
    Processes raw camera configuration into standardized specification dicts
    with all derived parameters computed (focal length, resolutions, etc.).
    
    Args:
        config: Raw camera configuration dictionary
        
    Returns:
        List of normalized camera specification dictionaries
...
def camera_feed_configs(config_path: str | None = None) -> list[dict]:
    ...
    Returns simplified camera feed information suitable for setting up
    ROS2 subscribers in external services (e.g., browser_controller).
    
    Args:
        config_path: Path to camera config YAML, or None for default
        
    Returns:
        List of camera feed configuration dictionaries
    """
...
def expected_video_keys(
    ...
    *,
    ...
    """Get expected video keys from data contract for embodiment.
    
    Args:
        contract_path: Path to data contract YAML, or None for default
        embodiment_key: Embodiment key to query
        
    Returns:
        List of expected video key strings
    """
...
def validate_camera_contract_alignment(
    ...
    *,
    ...
    """Validate camera specifications against data contract requirements.
    ...
    Args:
        camera_specs: List of normalized camera specifications
        contract_path: Path to data contract YAML, or None for default
        embodiment_key: Embodiment key to validate against
        
    Returns:
        Validation result dictionary with compliance status, errors, warnings
    """
