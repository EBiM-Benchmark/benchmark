...
def urdf_generation(
    ...
    if ABSOLUTE_PATHS and (HOST_DIR is None or HOST_DIR == ''):
        replacement_path = package_path
        robotiq_replacement_path = os.path.join(os.path.dirname(package_path), 'robotiq_description')
    elif ABSOLUTE_PATHS:
        replacement_path = HOST_DIR
        robotiq_replacement_path = os.path.join(
            os.path.dirname(HOST_DIR), 'ros2_robotiq_gripper', 'robotiq_description'
        )
    if ABSOLUTE_PATHS:
        urdf_file = convert_package_name_to_absolute_path(package_name, replacement_path, urdf_file)
        urdf_file = convert_package_name_to_absolute_path(
            'robotiq_description', robotiq_replacement_path, urdf_file
        )
...
if __name__ == '__main__':
    ...
    END_EFFECTORS = ['none', 'franka_hand', 'cobot_pump', 'robotiq_2f_85']
