...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Enable Kit-window dual-arm RMPflow control; always off headless.",
    )
...
def compensate_heading_yaw_rate(
    *,
    ...
    compensation = HEADING_HOLD_KP * yaw_error - HEADING_HOLD_KD * current_yaw_rate
...
def compute_base_drive_targets(
    ...
    wheel_vectors = [
        (vx - wz * y, vy + wz * x)
        for _steer, _drive, x, y in DRIVE_MODULES
    ]
    max_speed = max((math.hypot(*vector) for vector in wheel_vectors), default=0.0)
...
class DualArmRmpFlowTeleop:
    ...
    def _initialize_targets_from_fk(self) -> None:
        ...
        for arm in self._arms.values():
            ...
            active = arm["policy"].get_active_joints_subset().get_joint_positions()
    ...
    def _on_keyboard_event(self, event, *_args) -> bool:
        ...
        if event.type in (event_types.KEY_PRESS, event_types.KEY_REPEAT):
            ...
            for side, arm in self._arms.items():
                if event.input == arm["gripper_key"] and event.type == event_types.KEY_PRESS:
                    arm["gripper_open"] = not arm["gripper_open"]
                    print(f"{side} gripper toggled", flush=True)
            if event.input == self._reset_key and event.type == event_types.KEY_PRESS:
                for arm in self._arms.values():
    ...
    def _base_twist(self) -> tuple[float, float, float]:
        held_names = {
            self._base_keys[key] for key in self._held if key in self._base_keys
        }
    ...
    def _apply_base(
        ...
        yaw_rate = _wrap_to_pi(current_yaw - self._last_root_yaw) / max(dt, 1.0e-6)
        ...
        controller.apply_action(
            self._action_cls(
                ...
                joint_indices=np.asarray(self._steering_indices, dtype=np.int64),
            )
        )
        controller.apply_action(
            self._action_cls(
    ...
    def apply(self, dt: float) -> None:
        ...
        for arm in self._arms.values():
            ...
            actions.append(
                arm["policy"].get_next_articulation_action(dt)
            )
...
def _configure_robot_drives(stage, usd_physics) -> None:
    ...
    while pending:
        ...
        if gains is not None:
            ...
            drive_type = (
                "linear"
                if prim.IsA(usd_physics.PrismaticJoint)
                else "angular"
            )
...
def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        required.extend(config["description"] for config in ARM_CONFIGS.values())
        required.extend(config["rmpflow"] for config in ARM_CONFIGS.values())
        missing = [path for path in required if not path.is_file()]
        if missing:
            raise FileNotFoundError(
                "Missing Task 3 RMPflow assets: " + ", ".join(map(str, missing))
            )
        ...
        if arm_keyboard_enabled(args):
            ...
            print("Headless mode: arm keyboard teleop is disabled.", flush=True)
