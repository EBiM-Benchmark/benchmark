...
def test_cli_identifies_plain_isaac_sim_and_headless_disables_keyboard():
    ...
    assert scene.arm_keyboard_enabled(parser.parse_args(["--headless"])) is False
    assert scene.arm_keyboard_enabled(
...
def test_rmpflow_base_speed_defaults_match_requested_rates():
    ...
    assert scene.build_arg_parser().parse_args(
        ["--no-base-heading-hold"]
    ).base_heading_hold is False
...
def test_plain_isaac_sim_uses_task3_arm_spine_and_gripper_drive_gains():
    ...
    assert scene.robot_drive_gains("caster_front_left_joint") == (0.0, 0.0, 0.0)
    ...
    assert scene.robot_drive_gains("left_fr3v2_joint1") == (5000.0, 500.0, 200.0)
    assert scene.robot_drive_gains("right_fr3v2_joint7") == (5000.0, 500.0, 200.0)
    assert scene.robot_drive_gains("franka_spine_vertical_joint") == (
        5000.0,
        500.0,
        200.0,
    )
    assert scene.robot_drive_gains("left_fr3v2_finger_joint1") == (200.0, 20.0, 50.0)
...
def test_versioned_rmpflow_configs_use_current_hand_tcp_frames():
    ...
    assert scene.ARM_CONFIGS["left"]["end_effector_frame"] == "left_fr3v2_hand_tcp"
    assert scene.ARM_CONFIGS["right"]["end_effector_frame"] == "right_fr3v2_hand_tcp"
...
def test_shift_base_key_mapping_uses_requested_directions():
    assert scene.base_twist_from_held_key_names({"h"}) == pytest.approx((0, 0, 0))
    assert scene.base_twist_from_held_key_names({"shift", "h"}) == pytest.approx((1, 0, 0))
    assert scene.base_twist_from_held_key_names({"shift", "n"}) == pytest.approx((-1, 0, 0))
    assert scene.base_twist_from_held_key_names({"shift", "b"}) == pytest.approx((0, 1, 0))
    assert scene.base_twist_from_held_key_names({"shift", "m"}) == pytest.approx((0, -1, 0))
    assert scene.base_twist_from_held_key_names({"shift", "g"}) == pytest.approx((0, 0, 1))
    assert scene.base_twist_from_held_key_names({"shift", "j"}) == pytest.approx((0, 0, -1))
...
def test_forward_after_yaw_keeps_nearest_pi_target_and_waits_for_both_modules():
