...
EXPECTED_TASK3_PATHS = (
    ...
    "task3_isaacsim/scripts/run_task3_teleop.sh",
...
OLD_TASK3_ONLY_PATHS = (
    ...
    "scripts/config/task3_rmpflow",
    "scripts/config/task3_teleop",
...
def test_task3_participant_files_live_under_task_folder():
    missing = [
        path for path in EXPECTED_TASK3_PATHS if not (REPO_ROOT / path).is_file()
    ]
...
def test_old_task3_only_paths_are_removed():
    ...
    assert not remaining, f"Task-3-only files remain outside task folder: {remaining}"


def test_shared_room_builder_and_asset_remain_repository_wide():
    assert (REPO_ROOT / "scripts/scenes/scene_robot_room_keyboard.py").is_file()
