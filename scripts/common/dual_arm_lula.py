...
class DualArmLulaIK:
    ...
    def __init__(
        ...
        *,
        ...
        if not math.isfinite(warning_interval) or warning_interval < 0.0:
            raise ValueError("warning_interval must be finite and non-negative")
    ...
    def solve(
        ...
        base_position_array = _finite_vector(
            base_position, 3, "base_position"
        )
        self._set_robot_base_poses(base_position_array, base_orientation)
    ...
    def _validated_targets(
        ...
        if any(index < 0 or index >= len(self._joint_names) for index in indices):
        ...
        if set(targets) != set(expected_joints):
            raise ValueError(
                f"IK action joints do not match the expected arm: {sorted(targets)}"
            )
...
def _finite_vector(
    ...
    try:
        ...
        raise ValueError(f"{label} must contain {length} finite values") from None
