...
This is a modified version of the Task 2 recording script for Task3.
Self-contained LeRobot demonstration recorder for Task 3, run as the
`lerobot_recorder` docker compose service (profile `record`) defined in
[../../docker-compose.yml](../../docker-compose.yml).

| File | Purpose |
|---|---|
| `record_task3.py` | Interactive recorder node — subscribes to the Isaac Sim recording topics and writes a LeRobot dataset + `task2_extras/` sidecar |
...
```bash
task3_isaacsim/scripts/run_recorder.sh                       # defaults
task3_isaacsim/scripts/run_recorder.sh record --resume       # append to latest version
task3_isaacsim/scripts/run_recorder.sh record -- --fps 20    # one-off overrides
```
