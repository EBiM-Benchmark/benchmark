...
CI runs the same command on every pull request (`.github/workflows/pre-commit.yaml`), and `pre-commit` is the required status check on `main`, so a pull request cannot merge while it is red. The hooks cover Ruff (lint and format), codespell, license headers, and a set of file checks. Their configuration lives in `.pre-commit-config.yaml`, with Ruff's rules in `pyproject.toml`; several directories are excluded, listed under `exclude` at the end of `.pre-commit-config.yaml`. Not all of those exclusions are cosmetic — `.vscode/` is what stops the license-header hook from stamping an EBiM copyright onto `.vscode/tools/setup_vscode.py`, which carries Isaac Lab's own BSD-3-Clause header. Read [LICENSES/README.md](LICENSES/README.md) before changing that list.
